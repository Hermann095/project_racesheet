...
export function ResultsTableSectorsHead(props: ResultsTableSectorsProbs) {
    ...
    const showSectorBars = props.showSectorBars === undefined ? false : props.showSectorBars;
    ...
    return (
        ...
        (<TableCell>
          ...
          {sectors?.map((sector_time, index) => (
            ...
            <SectorCell 
              cellClass={sector_time.time === bestLap?.sector_times[index].time ? "fastest-time-cell" : (sector_time.state === "green" ? "personal-best-cell" : "slower-time-cell")}>
              </SectorCell> : null))}
          </div>
        </TableCell>)
        : (sectors?.map((sector_time, index) => (
            <TableCell align='center'>
            <span className={sector_time.time === bestLap?.sector_times[index].time ? "fastest-time" : (sector_time.state === "green" ? "personal-best" : "slower-time")}>
